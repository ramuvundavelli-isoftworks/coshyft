            ...
            <Card className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{(usage?.total_users ?? 0).toLocaleString()}</p>
                  <p className="text-xs text-gray-500">{usage?.active_tenants ?? 0} active tenants</p>
                </div>
              </div>
            </Card>
            <Card className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-600">Commutes Logged</p>
                  <p className="text-2xl font-bold text-gray-900">{(usage?.total_commutes_logged ?? 0).toLocaleString()}</p>
                </div>
              </div>
            </Card>
            <Card className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{(usage?.total_rides_completed ?? 0).toLocaleString()}</p>
                </div>
              </div>
            </Card>
            <Card className="p-6">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-yellow-100 rounded-lg"><BarChart3 className="h-5 w-5 text-yellow-600" /></div>
                <div>
                  <p className="text-sm text-gray-600">CO₂ Saved (kg)</p>
                  <p className="text-2xl font-bold text-gray-900">{(usage?.total_co2_saved_kg ?? 0).toLocaleString()}</p>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="p-6">
              ...
              {topTenants.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 py-8 text-center">No tenant data available</p>
              )}
            </Card>

            <Card className="p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Tenant User Breakdown</h3>
                ...
                {topTenants.map((t: any, idx: number) => (
                  <div key={t.tenant_id ?? idx} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{t.plan} · {t.status}</p>
                    </div>
                    <Badge className="bg-blue-100 text-blue-700">{(t.user_count ?? 0).toLocaleString()} users</Badge>
                  </div>
                ))}
                {topTenants.length === 0 && (
